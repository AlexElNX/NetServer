...
class Program
{
    ...
    {
        lock (MessagesLock)
            Messages.Add(new ChatMessage { From = clientId, Text = text, IsSystem = false, Time = DateTime.UtcNow });
    }

    static void AddSystemMessage(string text)
    {
        lock (MessagesLock)
            Messages.Add(new ChatMessage { From = "server", Text = text, IsSystem = true, Time = DateTime.UtcNow });
    }
